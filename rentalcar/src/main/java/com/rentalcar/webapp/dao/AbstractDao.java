...
{
	 @PersistenceContext
	 protected EntityManager entityManager; //grazie a questo possiamo eseguire le query, crea il contesto di persistenza

     protected final Class<I> entityClass;
     
     CriteriaBuilder builder; //per usare le criteria
	...
	public I SelById(Id id) {
		CriteriaQuery<I> query = this.InitCriteria();
		return this.entityManager.createQuery(query.where(
				builder.equal(
							query.from(this.entityClass).
							get("id"),id))).
				getSingleResult();
